...
async function normalizeCwd(cwd) {
  ...
  const stat4 = await fs.stat(resolved).catch(() => null);
  if (!stat4?.isDirectory()) throw new Error(`Directory does not exist: ${resolved}`);
  return resolved;
}
...
async function safeToolSection(fn) {
  try {
    ...
    return { ok: false, error: formatError2(error) };
  }
}
...
async function normalizeCwd2(cwd, deps) {
  ...
  const stat4 = await deps.fs.stat(resolved);
  if (!stat4?.isDirectory()) throw new Error(`Directory does not exist: ${resolved}`);
  return resolved;
}
...
async function listDevices(args = {}, dependencies) {
  ...
  return {
    ...
    async finish({ status, exitCode, payload, error }) {
      await writeJsonFile(recordPath, {
        ...
        error: error ? sanitizeErrorMessage2(formatError4(error)) : null
      });
    }
  };
}
...
async function findCommand(args, deps) {
  ...
  if (!cache) {
    return toolJson6({ available: false, reason: "No snapshot exists for the current session." });
  }
...
async function finderActionResult(args, deps) {
  ...
  if (action === "tap" || ["long-press", "fill", "scroll-into-view", "focus"].includes(action)) {
    return unwrapToolJson3(toolJson6(await planUnavailable(action)));
  }
  if (action === "inspect") {
...
function findMatches(refs, kind, value, name) {
  ...
  if (kind === "nth") {
    const index = clampNumber5(Number(value), 1, Number.MAX_SAFE_INTEGER) - 1;
    const needle = requireString4(name, "name");
...
function waitPredicate(args = {}) {
  ...
  if (args.fn !== void 0) return { kind: "fn", expression: requireString4(args.fn, "fn") };
  if (args.route !== void 0) return { kind: "route", route: requireString4(args.route, "route") };
  if (args.noSpinner === true) return { kind: "no-spinner" };
  if (args.text !== void 0) return { kind: "text", text: requireString4(args.text, "text") };
  if (args.ref !== void 0 || args.state !== void 0) {
    return {
      kind: "ref-state",
      ref: requireString4(args.ref, "ref"),
      state: requireString4(args.state ?? "visible", "state").toLowerCase()
    };
  }
  return null;
}
...
function batchStepError(error) {
  ...
  return {
    ...
    message: sanitizeErrorMessage3(formatError6(error)),
    exitCode
  };
}
...
function truncate6(value, limit = MAX_OUTPUT4) {
  const text = String(value ?? "");
  if (text.length <= limit) return text;
  return `${text.slice(0, limit)}
[truncated ${text.length - limit} characters]`;
...
function parseJsonArgument(value, flag) {
  try {
    ...
    throw new Error(`${flag} must be valid JSON: ${formatError6(error)}`);
  }
}
...
async function bootSimulator(args, deps) {
  ...
  const requestedDevice = optionalString2(args.device) ?? void 0;
...
async function launchApp(args, deps) {
  ...
  if (platform === "android") {
    const packageName = requireString5(args.packageName ?? args.bundleId, "packageName");
    const activity = optionalString2(args.activity);
  ...
  const bundleId = requireString5(args.bundleId ?? args.packageName, "bundleId");
  const device = await deps.resolveIosDevice(optionalString2(args.device) ?? void 0, { preferBooted: true });
...
async function terminateApp(args, deps) {
  const platform = platformArg(args.platform);
...
async function reloadApp(args, deps) {
...
async function iosCrashEvidence(args, deps) {
  ...
  const bundleId = optionalString2(args.bundleId);
  const processName = optionalString2(args.processName);
...
async function matchingIosCrashReports(args, deps) {
  const bundleId = optionalString2(args.bundleId);
  const processName = optionalString2(args.processName);
...
async function installApp(args, deps) {
  const platform = platformArg(args.platform);
  const appPath = resolvePath(requireString5(args.appPath, "appPath"));
...
async function uninstallApp(args, deps) {
  const platform = platformArg(args.platform);
...
async function resolveBundleId(args, deps) {
  const explicit = optionalString2(args.bundleId ?? args.packageName);
  if (explicit) return explicit;
  const cwd = optionalString2(args.cwd) ?? ".";
  const summary = await deps.runtimeSummary(cwd).catch(() => null);
  const inferred = optionalString2(summary?.appConfig?.iosBundleIdentifier ?? summary?.appConfig?.androidPackage);
  if (!inferred) throw new Error("bundleId must be provided or inferable from Expo app config.");
  return inferred;
}
async function collectAppLogs(args, deps) {
  const platform = platformArg(args.platform);
  if (platform === "android") {
    const device2 = optionalString2(args.device);
    const lines = String(clampNumber6(args.lines ?? 500, 1, 5e3));
  ...
  const device = await deps.resolveIosDevice(optionalString2(args.device) ?? void 0, { preferBooted: true });
  const last = optionalString2(args.last) ?? "2m";
  if (!/^\d+[smhd]$/.test(last)) throw new Error("last must look like 30s, 2m, 1h, or 1d.");
  const predicate = optionalString2(args.predicate) ?? iosLogPredicate(args);
...
function truncateSubprocessOutput(value, limit = MAX_OUTPUT5) {
...
function androidDeviceArgs(device, args) {
  const requested = optionalString2(device);
...
async function inferExpoScheme(cwd) {
  const appJsonPath = path3.join(cwd, "app.json");
  if (await pathExists2(appJsonPath)) {
    const appJson = await readJsonFile3(appJsonPath);
    const expo = isRecord5(appJson.expo) ? appJson.expo : {};
  ...
  if (!configPath) return null;
  const text = await fs3.readFile(configPath, "utf8");
...
async function resolveIosDevice(requested, options = {}, deps = {}) {
  ...
  const execFile4 = deps.execFile ?? defaultExecFile;
...
async function openUrl(args, deps = {}) {
  ...
  return toolJson8(redactToolPayload({ platform, device, stdout: truncate7(result.stdout), stderr: truncate7(result.stderr) }));
}
async function openExpoRoute(args, deps = {}) {
  ...
  return toolJson8(redactToolPayload({
    platform: "ios",
    device,
...
async function normalizeCwd3(cwd) {
  ...
  const stat4 = await fs3.stat(resolved).catch(() => null);
  if (!stat4?.isDirectory()) throw new Error(`Directory does not exist: ${resolved}`);
  return resolved;
}
async function findUp2(startDir, filename) {
  ...
  while (true) {
    const candidate = path3.join(current, filename);
...
var defaultExecFile = (file, args, options = {}) => new Promise((resolve15, reject) => {
...
async function automationTapInternal(args, deps, policyChecked) {
  ...
  if (args.ref) {
    ...
    const point = asRecord2(asRecord2(planned.plan).point);
  ...
  const x = String(clampNumber7(args.x, 0, Number.MAX_SAFE_INTEGER));
  const y = String(clampNumber7(args.y, 0, Number.MAX_SAFE_INTEGER));
  if (args.dryRun === true) {
  ...
  if (platform === "android") {
    ...
    return { platform, device: optionalString3(args.device), x: Number(x), y: Number(y), stdout: truncate8(result2.stdout), stderr: truncate8(result2.stderr) };
  }
  ...
  return { platform, device, tool: tool.tool, x: Number(x), y: Number(y), stdout: truncate8(result.stdout), stderr: truncate8(result.stderr) };
}
async function refActionCommand(args, deps) {
  ...
  if (command === "long-press" || command === "dbltap") {
    ...
    const coordinates = asRecord2(point.point);
  ...
  if (command === "drag") {
    ...
    return automationGestureInternal({
      ...args,
      gesture: "drag",
      startX: asRecord2(start.point).x,
      startY: asRecord2(start.point).y,
      endX: asRecord2(end.point).x,
      endY: asRecord2(end.point).y,
  ...
  if (command === "scroll") {
    ...
    return automationGestureInternal({ ...args, gesture: "swipe", ...asRecord2(plan.coordinates), durationMs: args.durationMs ?? 250 }, deps, true);
  }
  throw new Error(`Unknown ref action command: ${command}`);
}
async function clipboardCommand(args, deps) {
  ...
  if (action === "read") {
    ...
    return { available: !result2.error, action, device, text: result2.stdout, stderr: truncate8(result2.stderr), error: result2.error ?? null };
  }
  if (action === "write") {
    const text = requireString7(args.text, "text");
    const result2 = await deps.execFile("xcrun", ["simctl", "pbcopy", device.udid], { input: text, timeout: 1e4, rejectOnError: false });
    return { available: !result2.error, action, device, textLength: text.length, stdout: truncate8(result2.stdout), stderr: truncate8(result2.stderr), error: result2.error ?? null };
  }
  ...
  return { available: !result.error, action, device, tool: "axe", stdout: truncate8(result.stdout), stderr: truncate8(result.stderr), error: result.error ?? null };
}
async function keyboardCommand(args, deps) {
  ...
  if (action === "type") {
    ...
    return { available: !result2.error, action, device, tool: "axe", textLength: text.length, stdout: truncate8(result2.stdout), stderr: truncate8(result2.stderr), error: result2.error ?? null };
  }
  ...
  return { available: !result.error, action, device, tool: "axe", key, keycode, stdout: truncate8(result.stdout), stderr: truncate8(result.stderr), error: result.error ?? null };
}
function keyCodeFor(key) {
  ...
  if (/^\d+$/.test(normalized)) return clampNumber7(Number(normalized), 0, 255);
...
async function setEnvironmentCommand(args, deps) {
...
async function automationGesture(args, deps) {
...
async function automationGestureInternal(args, deps, policyChecked) {
  ...
  if (policyDenied) return policyDenied;
  const repeat = clampNumber7(args.repeat ?? 1, 1, 20);
  const intervalMs = clampNumber7(args.intervalMs ?? 250, 0, 1e4);
  const durationMs = clampNumber7(args.durationMs ?? defaultGestureDurationMs(gesture), 1, 3e4);
  const holdMs = args.holdMs === void 0 ? null : clampNumber7(args.holdMs, 0, 3e4);
  const metroPort = clampNumber7(args.metroPort ?? 8081, 1, 65535);
  const maxEvents = clampNumber7(args.maxEvents ?? 200, 1, 2e3);
  ...
  if (args.captureBeforeAfter === true) {
    asRecord2(evidence.screenshots).before = await captureGestureScreenshot({ platform, device: args.device, outputDir: args.outputDir, label: "before" }, deps);
  }
  ...
  if (args.captureBeforeAfter === true) {
    asRecord2(evidence.screenshots).after = await captureGestureScreenshot({ platform, device: args.device, outputDir: args.outputDir, label: "after" }, deps);
  }
  return {
...
function normalizeGestureCoordinates(gesture, args) {
  if (gesture === "tap" || gesture === "long-press") {
    return {
      x: clampNumber7(args.x, 0, Number.MAX_SAFE_INTEGER),
      y: clampNumber7(args.y, 0, Number.MAX_SAFE_INTEGER)
    };
  }
  return {
    startX: clampNumber7(args.startX, 0, Number.MAX_SAFE_INTEGER),
    startY: clampNumber7(args.startY, 0, Number.MAX_SAFE_INTEGER),
    endX: clampNumber7(args.endX, 0, Number.MAX_SAFE_INTEGER),
    endY: clampNumber7(args.endY, 0, Number.MAX_SAFE_INTEGER)
  };
}
function gestureCommandPlan(args) {
  const platform = platformArg2(args.platform);
  const gesture = requireString7(args.gesture, "gesture");
  const coordinates = asRecord2(args.coordinates);
...
async function executeGesturePlanInternal(args, deps, policyChecked) {
  ...
  const policyDenied = policyChecked ? null : await policyGate(args, `gesture.${gesture}`, "gesture", deps);
  if (policyDenied) return policyDenied;
  const repeat = clampNumber7(args.repeat ?? plan.repeat, 1, 20);
  const intervalMs = clampNumber7(args.intervalMs ?? plan.intervalMs, 0, 1e4);
  if (platform === "android") {
...
async function executeRepeatedCommandInternal(command, args, options, deps) {
  const repeat = clampNumber7(options.repeat ?? 1, 1, 20);
  const intervalMs = clampNumber7(options.intervalMs ?? 0, 0, 1e4);
  const runs = [];
  ...
  return {
    ...
    tool: options.tool ?? basename3(command),
    command: options.plannedCommand ?? [basename3(command), ...args],
    runs
  };
}
...
async function automationTakeScreenshot(args, deps = {}) {
  if (args.full === true) {
    return toolJson9(await (deps.captureFullScreenshot ?? captureFullScreenshot)(args, deps));
  }
  if (args.annotate === true) {
    return toolJson9(await (deps.annotatedScreenshot ?? annotatedScreenshot)(args, deps));
  }
  return toolJson9(await (deps.captureScreenshot ?? captureScreenshot)(args, deps));
}
...
async function execFilePromise2(file, args, options, deps = {}) {
  ...
  return new Promise((resolve15, reject) => {
    execFile2(file, args, { timeout: options.timeout, maxBuffer: options.maxBuffer ?? MAX_OUTPUT8 }, (error, stdout, stderr) => {
...
async function adbScreenshot(device, outputPath, deps) {
  ...
  await new Promise((resolve15, reject) => {
    ...
    child.on("close", (code) => {
      clearTimeout(timer);
      if (code === 0) {
        fs4.writeFile(outputPath, Buffer.concat(chunks2, byteLength)).then(resolve15, reject);
      } else {
...
async function defaultPathExists2(file, deps) {
  ...
  return pathExists3(file, { access: fs4.access });
}
...
async function writeFile3(file, contents, encoding, deps) {
  ...
  await fs4.writeFile(file, contents, encoding);
}
...
async function readDir(directory, deps) {
  ...
  return fs4.readdir(directory, { withFileTypes: true });
}
...
async function captureUxContext(args = {}, deps) {
  ...
  const metroSummary = args.includeRuntime === false ? { ok: false, skipped: true, reason: "includeRuntime is false" } : await safeToolSection3(() => deps.inspectMetro(metroPort, {
    ...
    componentFilter: requireOptionalString3(args.componentFilter)
  }));
  ...
  const inferredBundleId = requireOptionalString3(args.bundleId) ?? firstMetroAppId(context.metro) ?? appConfigBundleId(context.project) ?? null;
  const processName = requireOptionalString3(args.processName) ?? processNameFromBundleId2(inferredBundleId);
  if (inferredBundleId) {
...
function firstMetroAppId(metro) {
  const targets = asRecord4(metro)?.targets;
  if (!Array.isArray(targets)) return null;
  const target = targets.find((candidate) => asRecord4(candidate)?.appId);
  return typeof target?.appId === "string" ? target.appId : null;
}
function appConfigBundleId(project) {
  const bundleId = asRecord4(asRecord4(project)?.appConfig)?.iosBundleIdentifier;
...
async function annotateScreen(args = {}, deps) {
  ...
  const outputDir = deps.resolvePath(
    requireOptionalString4(args.outputDir) ?? deps.joinPath(cwd, ".scratch", "expo-ios-annotations", `annotation-${timestamp}`)
  );
  await deps.mkdir(outputDir, { recursive: true });
  const screenshotPath = deps.joinPath(outputDir, "screenshot.png");
  let context = null;
  const existingScreenshot = requireOptionalString4(args.screenshotPath);
  ...
  const title = requireOptionalString4(args.title) ?? "Expo screen annotations";
...
`, "utf8");
  ...
  let server = null;
  if (args.serve === true) {
    const port = args.port ? clampNumber10(args.port, 1, 65535) : await deps.findAvailablePort(17654);
...
async function runtimeInspector(args, deps) {
  ...
  const commentTitle = requireOptionalString5(args.commentTitle) ?? "Codex: Add UI comment";
  const maxComments = clampNumber11(args.maxComments ?? 50, 1, 500);
  if (action === "open-dev-menu") {
    return toolJson12(await deps.openIosDevMenu({ ...args, metroPort }));
  }
  const targets = await deps.fetchMetroTargets(metroPort).catch(() => []);
  const targetList = Array.isArray(targets) ? targets : [];
  const webSocketDebuggerUrl = asString(asRecord5(targetList[0])?.webSocketDebuggerUrl);
  if (!webSocketDebuggerUrl) {
    return toolJson12({ available: false, action, reason: "No Metro inspector target.", metroPort });
  }
  ...
  return toolJson12({
    ...
    protocolError: getPath(result, ["result", "exceptionDetails"]) ?? asRecord5(result)?.error ?? null,
    cdp: asRecord5(result)?.diagnostics ?? asRecord5(result)?.cdp ?? null
  });
}
...
function getPath(value, path8) {
  ...
  for (const part of path8) {
    current = asRecord5(current)?.[part];
...
async function reviewOverlay(args = {}, deps) {
  const action = requireOptionalString6(args.action) ?? "prepare";
  ...
  const cwd = await deps.normalizeProjectCwd(args.cwd, { allowMissingPackageJson: true }).catch(() => deps.resolvePath(String(args.cwd ?? deps.fallbackCwd())));
  const outputDir = deps.resolvePath(requireOptionalString6(args.outputDir) ?? deps.joinPath(cwd, ".scratch", "codex-review-overlay"));
  ...
  const title = requireOptionalString6(args.title) ?? "Codex in-app review";
...
async function scaffoldReviewOverlay(args = {}, deps) {
  const cwd = await deps.normalizeProjectCwd(args.cwd, { allowMissingPackageJson: true }).catch(() => deps.resolvePath(String(args.cwd ?? deps.fallbackCwd())));
  const overlayDir = deps.resolvePath(cwd, requireOptionalString6(args.overlayDir) ?? "codex-review-overlay");
...
async function reviewNextStep(args = {}) {
  ...
  const issue = requireOptionalString7(args.issue) ?? "unspecified UI review issue";
  const cwd = requireOptionalString7(args.cwd) ?? ".";
  const metroPort = clampNumber13(args.metroPort ?? 8081, 1, 65535);
  const componentFilter = requireOptionalString7(args.componentFilter);
  const verifierRule = requireOptionalString7(args.verifierRule);
...
async function annotationServer(args = {}, deps) {
  ...
  const port = clampNumber14(args.port ?? 17654, 1, 65535);
  await deps.listen({
    host: "127.0.0.1",
    port,
...
`, "utf8");
    ...
    return sendJsonPayload({ ok: false, error: formatError8(error) }, 500);
  }
}
...
async function devtoolsEventsPayload(args = {}, deps = {}) {
  ...
  await mkdir7(deps, eventsDir, { recursive: true });
  const file = joinPath(eventsDir, "events.json");
  const existing = await readJsonFile5(deps, file).catch(() => ({ events: [] }));
  const previousEvents = Array.isArray(asRecord6(existing)?.events) ? asRecord6(existing)?.events : [];
  const event = {
...
async function consoleCommand(args = {}, deps = {}) {
  return diagnosticMessagesCommand("console", args, deps);
}
async function errorsCommand(args = {}, deps = {}) {
...
async function diagnosticMessagesCommand(kind, args = {}, deps = {}) {
  ...
  if (action === "clear") {
    ...
    return toolJson15({
      ...
      action,
      metroPort,
      target: targetSummary2(targets[0]),
...
async function metroStatusPayload(args, deps) {
  ...
  return {
    ...
    version: asRecord6(version)?.__error ? null : version,
    versionError: asRecord6(version)?.__error ?? null,
    symbolication,
...
async function fetchMetroTargets(deps, metroPort) {
  const raw = await fetchJson(deps, `http://127.0.0.1:${metroPort}/json/list`, 2500).catch((error2) => ({
    __error: formatError9(error2)
  }));
  const error = asRecord6(raw)?.__error;
  ...
  return new Promise((resolve15) => {
    childProcess.execFile(file, args, { timeout: options.timeout }, (error, stdout, stderr) => {
      resolve15({
        stdout,
        stderr,
        error: error ? formatError9(error) : null
      });
    });
  });
}
function resolveExpoStateRoot4(args, deps) {
  ...
  return explicit ?? joinPath(typeof args.root === "string" ? args.root : ".", ".scratch", "expo-ios");
}
async function mkdir7(deps, dir, options) {
  if (deps.mkdir) return deps.mkdir(dir, options);
  const fs8 = await import("node:fs/promises");
  return fs8.mkdir(dir, options);
}
...
async function writeJsonFile3(deps, file, payload) {
  ...
  if (!deps.writeJsonFile) {
    const fs8 = await import("node:fs/promises");
    await fs8.writeFile(file, `${JSON.stringify(redacted, null, 2)}
`, "utf8");
    return void 0;
  }
...
async function probeMetroSymbolication(deps, metroPort) {
  try {
    ...
    return { available: false, endpoint: "/symbolicate", status: null, reason: formatError9(error) };
  }
}
async function fetchText(deps, url, timeoutMs) {
  try {
    ...
    return { available: false, text: null, error: formatError9(error) };
  }
}
...
function normalizeMetroTarget(value, index) {
  ...
  const target = {
    ...
    reactNative: asRecord6(record.reactNative),
...
async function metroCommand(args = {}, deps = {}) {
  ...
  if (action === "reload") return toolJson16(await (deps.metroReloadPayload ?? ((nextArgs) => metroReloadPayload(nextArgs, deps)))(args));
  if (action === "symbolicate") {
    return toolJson16(await (deps.metroSymbolicatePayload ?? ((nextArgs) => metroSymbolicatePayload(nextArgs, deps)))(args));
  }
  if (action !== "status") throw new Error(`Unknown metro action: ${action}`);
  return toolJson16(await (deps.metroStatusPayload ?? ((nextArgs) => metroStatusPayload2(nextArgs, deps)))(args));
}
...
var MetroInspectorClient = class {
  ...
  async status() {
    try {
      ...
      return { available: false, endpoint: "/status", text: null, error: formatError10(error) };
    }
  }
  async version() {
    try {
      ...
      return { available: false, endpoint: "/json/version", value: null, error: formatError10(error) };
    }
  }
  async targets() {
    let raw;
  ...
  async statusPayload() {
    ...
    return {
      ...
      targets: targetsResult.targets.map(targetSummary3),
      targetDiscovery: {
        endpoint: "/json/list",
...
async function defaultEvaluateHermesExpression(webSocketDebuggerUrl, expression, { timeoutMs = 3e3 } = {}) {
  ...
  try {
    ...
    return { error: formatError10(error) };
  } finally {
    client.close();
  }
}
var HermesCdpClient = class {
  ...
  handleMessage(event) {
    ...
    const record = asRecord7(payload);
    if (!record || typeof record.id !== "number") return;
    const pending = this.pending.get(record.id);
    if (!pending) return;
    clearTimeout(pending.timeout);
    this.pending.delete(record.id);
    pending.resolve(record.error ? { error: formatError10(record.error) } : record);
  }
};
...
async function navigationCommand(args = {}, deps = {}) {
  ...
  if (action === "deep-link") return toolJson17(await navigationDeepLink(args, deps));
  ...
  if (!webSocketDebuggerUrl) {
    return toolJson17(navigationUnavailable({ action, metroPort, reason: "No Metro inspector target.", policy }));
  }
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return toolJson17(navigationUnavailable({
      action,
      metroPort,
...
async function networkCommand(args = {}, deps = {}) {
  ...
  const webSocketDebuggerUrl = target?.webSocketDebuggerUrl ?? null;
  if (!webSocketDebuggerUrl) {
    return toolJson18(networkUnavailable({
      action: bridgeAction,
      metroPort,
...
var defaultFileSystem = {
  mkdir: (filePath, options) => fs6.mkdir(filePath, options).then(() => void 0),
  writeJsonFile: (filePath, value) => fs6.writeFile(filePath, `${JSON.stringify(value, null, 2)}
`, "utf8")
};
...
async function storageCommand(args = {}, deps = {}) {
  ...
  if (!policy.allowed) return toolJson19(policyDeniedPayload3({ domain: "storage", action, policy }));
  const value = action === "set" ? parseStorageValue(args.value ?? positionals[3]) : null;
  return toolJson19(await bridgeDomainCommand({
    args,
    domain: "storage",
    action,
...
async function stateCommand(args = {}, deps = {}) {
  ...
  if (!policy.allowed) return toolJson19(policyDeniedPayload3({ domain: "state", action, policy }));
  return toolJson19(await bridgeDomainCommand({
    args,
    domain: "state",
    action,
...
async function controlsCommand(args = {}, deps = {}) {
  ...
  if (!policy.allowed) return toolJson19(policyDeniedPayload3({ domain: "controls", action, policy }));
  return toolJson19(await bridgeDomainCommand({
    args,
    domain: "controls",
    action,
...
function domainUnavailable(args) {
  return sanitizePayload2({
    ...
    reason: args.reason,
    metroPort: args.metroPort,
    target: targetSummary6(args.target),
...
async function bridgeCommand(args = {}, dependencies = {}) {
  ...
  if (action === "status") return toolJson20({ available: true, action, ...status });
  if (action === "plan") return toolJson20({ available: true, action, status: status.state, projectRoot: status.projectRoot, plan });
  if (action === "health" || action === "domains") {
    return toolJson20(await io.bridgeHealthPayload(args, { action, status, plan }));
  }
  ...
  if (action === "install") {
    ...
    return toolJson20({ available: true, action, projectRoot: cwd, installed: true, status: (await bridgeInstallStatus(cwd, io)).state, plan });
  }
  ...
  return toolJson20({ available: true, action, projectRoot: cwd, removed: true, status: (await bridgeInstallStatus(cwd, io)).state, plan });
}
...
async function normalizeProjectCwd2(cwd, options = {}) {
  ...
  const stat4 = await fs7.stat(resolved).catch(() => null);
...
async function pathExists4(file) {
  return fs7.access(file).then(() => true, () => false);
}
async function writeJsonFile4(file, value) {
  await fs7.writeFile(file, `${JSON.stringify(value, null, 2)}
`, "utf8");
}
function bridgeCommandIo(dependencies) {
  return {
    ...
    mkdir: dependencies.mkdir ?? fs7.mkdir,
    writeJsonFile: dependencies.writeJsonFile ?? writeJsonFile4,
    writeFile: dependencies.writeFile ?? fs7.writeFile,
    rm: dependencies.rm ?? fs7.rm,
...
function dependencyMap(packageJson) {
  const record = asRecord9(packageJson);
  return {
    ...asRecord9(record?.dependencies),
    ...asRecord9(record?.devDependencies)
  };
}
function metadataProperty(metadata, key) {
  return asRecord9(metadata)?.[key];
}
...
async function accessibilityCommand(args = {}, deps = {}) {
  const positionals = Array.isArray(args._) ? args._ : [];
  ...
  if (action === "inspect") {
    ...
    if (!cache) return toolJson21({ available: false, action, reason: "No snapshot exists for the current session.", ref });
    const record = (cache.refs ?? []).find((item) => item.ref === ref);
    return toolJson21(record ? { available: true, action, ref, snapshotId: cache.snapshotId, targetId: cache.targetId, record } : { available: false, action, reason: "Ref not found in the latest snapshot.", ref });
  }
  if (action === "audit") {
    const cache = await readLatestRefCache3(args, deps);
    if (!cache) return toolJson21({ available: false, action, reason: "No snapshot exists for the current session.", issues: [] });
    const issues = auditAccessibilityRefs(cache);
    return toolJson21({ available: true, action, snapshotId: cache.snapshotId, targetId: cache.targetId, issueCount: issues.length, issues });
  }
  return toolJson21(await accessibilityTreePayload(args, deps));
}
...
async function readLatestRefCache3(args = {}, deps = {}) {
  ...
  const session = asRecord10(await readLatestSession3(stateRoot));
...
async function readLatestSession3(stateRoot) {
  ...
  sessions.sort((a, b) => String(asRecord10(b)?.updatedAt ?? asRecord10(b)?.createdAt).localeCompare(String(asRecord10(a)?.updatedAt ?? asRecord10(a)?.createdAt)));
  return sessions[0] ?? null;
}
...
async function modalBridgeCommand(input, deps) {
  ...
  return toolJson22(await bridgeDomainCommand2({
    args: input.args,
    domain: input.domain,
    action,
...
function stringifyBoundedJson2(value) {
  ...
  const record = asRecord11(sanitized);
  const envelope = {
    available: false,
    source: "output-boundary",
    evidenceSource: "output-boundary",
...
async function recordCommand(args = {}, deps = {}) {
  ...
  await mkdir8(recordDir, { recursive: true });
  ...
  if (action === "start") {
    ...
    return toolJson23({ ...metadata2, metadataPath });
  }
...
async function diffCommand(args = {}, deps = {}) {
  ...
  const payload = {
    ...diff,
    kind,
    sessionId: asRecord13(session)?.sessionId ?? null,
    targetId: asRecord13(session)?.activeTargetId ?? null,
    outputPath
  };
  await writeJsonFile6(outputPath, payload);
  return toolJson24(payload);
...
function reviewReportPayload(args) {
  const session = asRecord13(args.session);
...
async function snapshotDiffPayload(args = {}) {
  ...
  return {
    available: true,
    baselineSnapshotId: asRecord13(baseline)?.snapshotId ?? null,
    currentSnapshotId: asRecord13(current)?.snapshotId ?? null,
...
async function screenshotDiffPayload(args = {}) {
  ...
  const [before, after] = await Promise.all([stat3(baseline), stat3(current)]);
  return {
    available: true,
    baseline,
    current,
...
async function latestSnapshotJson(args = {}) {
  ...
  const stateRoot = resolveExpoStateRoot7(args);
  const session = await readLatestSession5(stateRoot);
  const sessionId = asRecord13(session)?.sessionId;
...
async function listRunRecords(stateRoot) {
  ...
  const entries = await readdir5(runsRoot, { withFileTypes: true }).catch(() => []);
  const records = [];
  for (const entry of entries) {
    if (!entry.isFile() || !entry.name.endsWith(".json")) continue;
    const file = join8(runsRoot, entry.name);
    const record = asRecord13(await readJsonFile9(file).catch(() => null));
...
function refsFromSnapshot(snapshot) {
  const refs = asRecord13(snapshot)?.refs;
  if (!Array.isArray(refs)) return [];
  return refs.map((record) => asRecord13(record)?.ref).filter((ref) => typeof ref === "string");
}
...
async function debugInspectCommand(args = {}, deps = {}) {
  return toolJson25(await debugInspectPayload(args, deps));
}
...
async function highlightCommand(args = {}, deps = {}) {
  const ref = requireString20(args.ref ?? firstPositional(args), "ref");
  const found = await readRefRecord(ref, args, deps);
  if (found.available === false) return toolJson25({ ...found, action: "highlight" });
...
async function readLatestSession6(stateRoot) {
  ...
  return asRecord14(sessions[0]);
}
async function readSelectedTarget(stateRoot, session) {
  return readJsonFile10(join9(sessionDirectory4(stateRoot, String(session.sessionId)), "target.json")).then(asRecord14).catch(() => null);
}
...
function unwrapToolJson9(value) {
  const text = asRecord15(value)?.content;
  if (!Array.isArray(text)) return value;
  const first = asRecord15(text[0]);
...
async function expoCommand(args = {}, deps) {
  ...
  if (action === "upstream-policy") {
    const info = asRecord15(unwrapToolJson9(await deps.projectInfo({ cwd: summary.projectRoot }))) ?? {};
    return toolJson26({
...
async function expoModuleRecords(projectRoot, deps) {
  ...
  const packageJson = packageJsonPath ? asRecord15(await deps.readJsonFile(packageJsonPath)) ?? {} : {};
  const allDeps = {
    ...asRecord15(packageJson.dependencies),
    ...asRecord15(packageJson.devDependencies)
  };
...
async function readExpoAppConfigPlugins(projectRoot, deps) {
  ...
  if (await deps.pathExists(appJsonPath)) {
    const appJson = asRecord15(await deps.readJsonFile(appJsonPath));
    const expoConfig = asRecord15(appJson?.expo);
...
async function rnCommand(args = {}, deps = {}) {
  const positionals = Array.isArray(args._) ? args._ : [];
  ...
  if (action === "inspect") return toolJson27(await rnInspectPayload(args, deps));
...
async function perfCommand(args = {}, deps = {}) {
  ...
  if (action === "summary") return toolJson28(await perfSummaryPayload(args, deps));
  if (action === "bundle") return toolJson28(await perfBundlePayload(args, deps));
  if (action === "compare") return toolJson28(await perfComparePayload(args, deps));
  if (action === "budget") return toolJson28(await perfBudgetPayload(args, deps));
  if (action === "memory") return toolJson28(await perfMemoryPayload(args, deps));
  if (action === "ettrace" || action === "memgraph") return toolJson28(await perfNativeProfilerPayload(args, action, deps));
  if (["mark", "measure", "js-thread", "frames"].includes(action)) return toolJson28(await perfInstrumentedPayload(args, action, deps));
  return toolJson28(await perfRuntimePayload(args, action, deps));
}
...
async function perfRuntimePayload(args = {}, action, deps = {}) {
  ...
  const metro = target ? { available: true, metroPort, status: "available", statusText: null, targetCount: targets.length, targets: targets.map(targetSummary8) } : await metroStatus2({ metroPort }, deps);
...
async function perfInstrumentedPayload(args = {}, action, deps = {}) {
  const subaction = requireOptionalString8(args.subaction);
  const label = requireOptionalString8(args.label);
  const bridgeAction = perfBridgeAction(action, subaction);
  const metroPort = clampNumber22(args.metroPort ?? 8081, 1, 65535);
  const targets = await listMetroTargets(metroPort, deps);
  const target = targets[0] ?? null;
  const projectRoot = await projectCwd(args.cwd, deps);
  const metro = target ? { available: true, metroPort, status: "available", targetCount: targets.length, targets: targets.map(targetSummary8) } : await metroStatus2({ metroPort }, deps);
  let bridgePayload = null;
  if (target?.webSocketDebuggerUrl) {
...
async function perfMemoryPayload(args = {}, deps = {}) {
  ...
  const nativeArtifact = requireOptionalString8(args.nativeArtifact);
  const projectRoot = await projectCwd(args.cwd, deps);
...
async function perfBundlePayload(args = {}, deps = {}) {
  const cwd = await projectCwd(args.cwd, deps);
  const bundleArtifact = requireOptionalString8(args.bundleArtifact);
  const metrics = [];
  const unavailableSources = [];
  let available = false;
  let bundlePath = null;
  if (bundleArtifact) {
    ...
    const stat4 = await fileStat(bundlePath, deps);
    if (stat4?.isFile()) {
      available = true;
      metrics.push(perfMetric({ name: "bundle.bytes", value: stat4.size, unit: "bytes", source: "metro", confidence: "high" }));
    } else {
...
function perfTransport(metroPort, target, cdp = null) {
  return { name: "metro-inspector-hermes-cdp", metroPort, protocol: "Runtime.evaluate", target: targetSummary8(target), cdp };
}
...
function normalizePerfBuildKind(value) {
  const buildKind = requireOptionalString8(value) ?? "development";
...
async function evaluateHermes(url, expression, deps) {
  return deps.evaluateHermesExpression ? deps.evaluateHermesExpression(url, expression, { timeoutMs: 5e3 }) : {};
}
...
async function dashboardCommand(args = {}) {
  ...
  await mkdir10(dashboardDir, { recursive: true });
  const previous = asRecord17(await readJsonFile12(statePath).catch(() => null));
  const previousArtifacts = asRecord17(previous?.artifacts);
...
async function writeDashboardHtml(file, payload) {
  await mkdir10(dirname8(file), { recursive: true });
...
async function policyCommand(args = {}) {
  ...
  const policyPath = requireOptionalString9(args.actionPolicy);
...
async function redactCommand(args = {}) {
  ...
  const outputPath = requireOptionalString9(args.outputPath);
  const resolvedOutputPath = outputPath ? resolve12(outputPath) : null;
  if (resolvedOutputPath) {
    await mkdir11(dirname9(resolvedOutputPath), { recursive: true });
...
async function skillsCommand(args = {}, deps = {}) {
  ...
  if (!skill) return toolJson31({ available: false, action, name, reason: "Skill not found.", pluginVersion: CLI_VERSION6 });
  return toolJson31({ available: true, action, pluginVersion: CLI_VERSION6, ...skill });
}
async function listBundledSkills(deps = {}) {
  const skillsRoot = join13(pluginRoot(deps), "skills");
  const entries = await readdir9(skillsRoot, { withFileTypes: true }).catch(() => []);
  const skills = [];
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const file = join13(skillsRoot, entry.name, "SKILL.md");
    const content = await readFile13(file, "utf8").catch(() => null);
    if (!content) continue;
...
async function installCommand(args = {}, deps = {}) {
  ...
  const prefix = resolve13(optionalString7(args.prefix) ?? join13(deps.homeDir ?? homedir(), ".local"));
...
async function releaseCommand(args = {}, deps = {}) {
  ...
  await mkdir12(outsideCwd, { recursive: true });
  const fixture = join13(outsideCwd, "routes-fixture");
  await mkdir12(join13(fixture, "app"), { recursive: true });
...
async function writeJsonFile9(file, value) {
  await mkdir12(dirname10(file), { recursive: true });
...
async function liveBacklogCommand(args = {}, deps = {}) {
  ...
  if (action === "self-check") {
    return toolJson32({ available: selfCheck.ok, action, cwd, scope, selfCheck, source: matrix.source, rowCount: matrix.rows.length });
  }
  if (action === "matrix") {
    return toolJson32({ available: true, action, cwd, scope, source: matrix.source, selfCheck, rowCount: matrix.rows.length, rows: matrix.rows });
  }
  if (!selfCheck.ok) {
    return toolJson32({ available: false, action, cwd, scope, source: matrix.source, selfCheck, reason: "Live backlog self-check failed before executing rows." });
  }
  ...
  return toolJson32({ ...report, reportPath });
}
...
async function traceInteraction(args = {}, deps) {
  ...
  const maxEvents = clampNumber24(args.maxEvents ?? 300, 1, 2e3);
  const includeEvents = args.includeEvents === true;
  const componentFilter = requireOptionalString10(args.componentFilter);
  const targets = await deps.fetchMetroTargets(metroPort).catch(() => []);
  const targetList = Array.isArray(targets) ? targets : [];
  const webSocketDebuggerUrl = asString2(asRecord18(targetList[0])?.webSocketDebuggerUrl);
  if (!webSocketDebuggerUrl) {
    return toolJson33({
      available: false,
      action,
  ...
  return toolJson33({
    ...
    trace: getPath2(result, ["result", "result", "value"]) ?? null,
    protocolError: getPath2(result, ["result", "exceptionDetails"]) ?? asRecord18(result)?.error ?? null,
    cdp: asRecord18(result)?.diagnostics ?? asRecord18(result)?.cdp ?? null
  });
}
...
function targetSummary9(target) {
  const record = asRecord18(target);
  if (!record) return null;
  return {
    title: record.title,
    appId: record.appId,
...
function getPath2(value, path8) {
  ...
  for (const key of path8) {
    current = asRecord18(current)?.[key];
  }
  return current;
}
