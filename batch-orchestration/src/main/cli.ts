...
import type { ParsedCliArgs } from "./domain.js";
import { formatError } from "./errors.js";

export function parseCliArgs(argv: string[]): ParsedCliArgs {
  const args: Record<string, unknown> & { _: unknown[] } = { _: [] };
...
function normalizeGlobalFlag(rawKey: string): string | null {
  switch (rawKey) {
    case "json":
    case "plain":
    case "quiet":
...
function globalFlagTakesValue(rawKey: string): boolean {
  return rawKey === "root"
    || rawKey === "state-dir"
