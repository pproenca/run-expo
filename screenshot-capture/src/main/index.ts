...
export async function captureFullScreenshot(
  ...
  const outputPath = path.resolve(
    args.outputPath ??
      path.join(os.tmpdir(), "expo-ios-screenshots", `full-screenshot-${safeTimestamp(deps)}.png`),
  );
...
export async function captureScreenshot(
  ...
  const outputPath = path.resolve(
    args.outputPath ??
      path.join(os.tmpdir(), "expo-ios-screenshots", `screenshot-${safeTimestamp(deps)}.png`),
  );
  await mkdir(path.dirname(outputPath), deps);
