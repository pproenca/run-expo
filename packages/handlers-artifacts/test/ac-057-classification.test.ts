...
describe("AC-057 live-backlog row classification (fake live-evidence signal)", () => {
  it("AC-057 exit 2 → expected-usage-error (even with requirements + live evidence)", () => {
    expect(
      classifyRow(evidence({ exitCode: 2, requirements: REQ, liveEvidence: LIVE }))
    ).toBe("expected-usage-error")
  })

  it("AC-057 non-zero exit WITH requirements → environment-blocked", () => {
    expect(classifyRow(evidence({ exitCode: 1, requirements: REQ }))).toBe(
      "environment-blocked"
    )
  })

  it("AC-057 non-zero exit with NO requirements → defect", () => {
    expect(classifyRow(evidence({ exitCode: 1, requirements: NO_REQ }))).toBe(
      "defect"
    )
  })
  ...
  it("AC-057 exit 0 + runtime requirement but NO live evidence → environment-blocked", () => {
    expect(
      classifyRow(evidence({ exitCode: 0, requirements: REQ, liveEvidence: NO_LIVE }))
    ).toBe("environment-blocked")
  })

  it("AC-057 exit 0 + runtime requirement + live evidence → live-pass", () => {
    expect(
      classifyRow(evidence({ exitCode: 0, requirements: REQ, liveEvidence: LIVE }))
    ).toBe("live-pass")
  })

  it("AC-057 exit 0 + NO requirements → static-pass", () => {
    expect(
      classifyRow(evidence({ exitCode: 0, requirements: NO_REQ }))
    ).toBe("static-pass")
  })
  ...
  it("AC-057 available:false with a designed code → designed-unavailable", () => {
    expect(
      classifyRow(
        evidence({ availableFalse: true, unavailableCode: "policy-denied", requirements: REQ })
      )
    ).toBe("designed-unavailable")
  })
  ...
  it("AC-057 available:false, unknown code, WITH requirements → environment-blocked", () => {
    expect(
      classifyRow(
        evidence({ availableFalse: true, unavailableCode: "mystery", requirements: REQ })
      )
    ).toBe("environment-blocked")
  })
  ...
  it.effect(
    "AC-057 live-backlog run classifies each substituted row from injected evidence",
    () =>
      Effect.gen(function* () {
        ...
        const byId = Object.fromEntries(
          payload.rows.map((r) => [r.id, r.classification])
        )
  ...
  it.skip(
    "AC-057 live-backlog run against a REAL environment (Metro + Hermes + simulator)",
    () => {
