...
describe("D12 small read commands (envelope/shape)", () => {
  it.effect("diff with no --baseline is available:false (designed-unavailable)", () =>
    Effect.gen(function* () {
      const result = yield* dispatch(diffCommand("snapshot"), {}).pipe(
        Effect.provide(Caps)
      )
  ...
  it.effect("diff with a --baseline compares candidate vs baseline", () =>
    Effect.gen(function* () {
      const result = yield* dispatch(
        diffCommand("screenshot", { baseline: "a", candidate: "b" }),
        {}
      ).pipe(Effect.provide(Caps))
      const payload = result.payload as DiffResult
      expect(payload.available).toBe(true)
      expect(payload.changed).toBe(true)
      const same = yield* dispatch(
        diffCommand("screenshot", { baseline: "a", candidate: "a" }),
        {}
      ).pipe(Effect.provide(Caps))
  ...
  it.effect("ux-context reflects the --include-* facets in order", () =>
    Effect.gen(function* () {
      const result = yield* dispatch(
        uxContextCommand({ includeScreenshot: true, includeLogs: true }),
        {}
      ).pipe(Effect.provide(Caps))
  ...
  it.effect("review-next echoes the --surface/--stage/--issue context as steps", () =>
    Effect.gen(function* () {
      const result = yield* dispatch(
        reviewNextCommand({ surface: "home", stage: "triage", issue: "BUG-1" }),
        {}
      ).pipe(Effect.provide(Caps))
  ...
  it.effect("review report/matrix render the captured entries", () =>
    Effect.gen(function* () {
      const report = yield* dispatch(
        reviewCommand("report", { entries: [{ x: 1 }, { y: 2 }] }),
        {}
      ).pipe(Effect.provide(Caps))
      ...
      const matrix = yield* dispatch(reviewCommand("matrix"), {}).pipe(
        Effect.provide(Caps)
      )
