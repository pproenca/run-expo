...
injection**. The side-effect class is a *required typed field* on each command;
...
matching capability. These directives make `tsc --noEmit` pass *because* the
...
`runtime-eval` class**. Any handler that tries to evaluate JS therefore *must*
