...
const run = <A>(
  ...
  >
): Effect.Effect<DispatchResult<A>> =>
  dispatch(cmd as Command<SideEffect, A>, policy).pipe(Effect.provide(caps))
...
describe("AC-005 lifecycle mutations are policy-gated", () => {
  ...
  for (const verb of ALL_VERBS) {
    it.effect(
      `AC-005 ${verb} with NO policy is DENIED and the device capability is NEVER invoked (0 calls)`,
      () =>
        Effect.gen(function* () {
...
describe("AC-029 launch/reload attach crashCheck and fail closed on crash", () => {
  ...
  it.effect("AC-029 launch-app fails closed (available:false) when a matching crash report appears after startedAt", () =>
    Effect.gen(function* () {
      const calls = yield* Ref.make<ReadonlyArray<string>>([])
