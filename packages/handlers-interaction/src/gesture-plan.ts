 ...
 */
export const pointForBox = (box: ScreenBox | null): ScreenPoint | null =>
  box === null ? null : centerOf(box)
...
export const resolveScrollAmount = (args: ScrollArgs): number =>
  clamp(
    args.amount ?? args.text ?? DEFAULT_SCROLL_AMOUNT,
    MIN_SCROLL_AMOUNT,
    MAX_SCROLL_AMOUNT
  )
...
export const planGesture = (
  ...
  const to: ScreenPoint =
    kind === "swipe" || kind === "drag"
      ? { x: args.toX ?? from.x, y: args.toY ?? from.y }
      : from
  return {
    kind,
    from,
