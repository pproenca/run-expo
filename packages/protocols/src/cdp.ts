...
export type CdpEvaluateResult =
  | { readonly available: true; readonly result: CdpEvaluation }
  | CdpUnavailable;
...
export const HermesEvidence = Context.GenericTag<HermesEvidence>(
  "@expo98/protocols/HermesEvidence",
);
...
export const HermesRuntimeEval = Context.GenericTag<HermesRuntimeEval>(
  "@expo98/protocols/HermesRuntimeEval",
);
...
const parseFrame = (raw: string): Effect.Effect<CdpFrame, CdpMalformedFrame> =>
  Effect.try({
    try: () => JSON.parse(raw) as CdpFrame,
      ...
      new CdpMalformedFrame({ rawTruncated: truncate(raw, MALFORMED_PREVIEW_CHARS) }),
  });
...
const runEvaluateSequence = (
  ...
  Effect.gen(function* () {
    ...
    const send = (method: string, params?: Record<string, unknown>) => {
      const id = ++nextId;
      return socket
        .send(JSON.stringify({ id, method, params: params ?? {} }))
        .pipe(Effect.as(id));
    };
    ...
    ): Effect.Effect<A, E | CdpSocketError> =>
      eff.pipe(
        Effect.timeoutFail({
          duration: timeoutMs,
          onTimeout: () =>
    ...
    const result = frame.result as
      | { result?: { value?: unknown }; value?: unknown }
      | undefined;
      ...
      const attempt = Effect.scoped(
        Effect.gen(function* () {
          const socket = yield* Effect.acquireRelease(
            factory.connect({ url, origin, openTimeoutMs }),
            (s) => s.close,
          );
...
const makeEvidence = Effect.gen(function* () {
  ...
  return {
    evaluateReadOnly: (expression, options) =>
      evaluateOverCandidates(factory, expression, options),
...
export const assertLoopbackUrl = (url: string): Effect.Effect<string, LoopbackViolation> => {
  ...
  return Effect.fail(
    new LoopbackViolation({ host: res.host, url, reason: res.reason ?? "Non-loopback host." }),
  );
};

/** Re-export for the test: bounded-open is a clamp to [.., 2500]. */
export const __boundedOpenClamp = (timeoutMs: number): number =>
  clamp(boundedOpenMs(timeoutMs), 0, MAX_OPEN_MS);
