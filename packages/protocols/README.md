...
> **INTEGRATION SEAM (`@expo98/core`)** — runtime-eval is gated by core's dispatcher via *capability
> injection*. This package only **documents** the seam; it does **not** implement the gate.
...
`runtime-eval`-classed command. A `read`-classed handler's `R` simply *lacks* `HermesRuntimeEval`, so
calling it is a **compile error** — the gate is a type fact, not a runtime convention. This package
ships both tags + layers; the *withholding* is core's job.
...
stronger: the base URL is *constructed* as `http://127.0.0.1:<clamped-port>`, never taken from the
