...
describe("AC-014 routing — GET /events.json + 404 default", () => {
  it.effect("GET /events.json with NO file → 200 {\"events\":[]}", () =>
    Effect.gen(function* () {
      const res = yield* handleRequest(
        { method: "GET", url: "/events.json", headers: {}, body: "" },
        config
      )
      expect(res.status).toBe(200)
  ...
  it.effect("GET an unknown path → 404", () =>
    Effect.gen(function* () {
      const res = yield* handleRequest(
        { method: "GET", url: "/whatever", headers: {}, body: "" },
        config
      )
      expect(res.status).toBe(404)
    }).pipe(Effect.provide(testLayer))
  ...
  it.effect("an unsupported method on the endpoint → 404", () =>
    Effect.gen(function* () {
      const res = yield* handleRequest(
        { method: "DELETE", url: "/events", headers: baseHeaders, body: "" },
        config
      )
      expect(res.status).toBe(404)
    }).pipe(Effect.provide(testLayer))
...
describe("AC-014 live HTTP bind (skipped — needs a real socket)", () => {
  it.skip(
    "AC-014 live loopback bind + EADDRINUSE port search round-trip (127.0.0.1, from 17655)",
    () => {}
  )
})
