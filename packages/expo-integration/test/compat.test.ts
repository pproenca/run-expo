...
import { classifyCompat, type CompatMap, DEFAULT_COMPAT_MAP, parseVersion } from "@expo98/expo-integration"

describe("AC-020 Expo↔RN compat classification (data-file map)", () => {
  it("AC-020 missing — either side absent or blank", () => {
    expect(classifyCompat({ expo: undefined, reactNative: "0.81.0" }).classification).toBe("missing")
    expect(classifyCompat({ expo: "54.0.0", reactNative: null }).classification).toBe("missing")
    expect(classifyCompat({ expo: "   ", reactNative: "0.81" }).classification).toBe("missing")
  })

  it("AC-020 declared-unresolved — manifest pointer prefixes", () => {
    for (const prefix of ["catalog:", "workspace:", "file:", "link:", "portal:"]) {
      expect(classifyCompat({ expo: `${prefix}expo`, reactNative: "0.81.0" }).classification).toBe(
        "declared-unresolved",
      )
      expect(classifyCompat({ expo: "54.0.0", reactNative: `${prefix}rn` }).classification).toBe("declared-unresolved")
    }
  })

  it("AC-020 unknown — Expo major not in the table", () => {
    // 99 is not a key in the bundled data file.
    expect(classifyCompat({ expo: "99.0.0", reactNative: "0.99" }).classification).toBe("unknown")
  })
  ...
  it("AC-020 covers every bundled row", () => {
    ...
    for (const [expo, rn] of Object.entries(expected)) {
      expect(classifyCompat({ expo: `${expo}.0.0`, reactNative: `${rn}.3` }).classification).toBe("compatible")
    }
  })
  ...
  it("AC-020 data-file extensibility — an Expo 55 manifest row classifies WITHOUT a code change", () => {
    ...
    expect(classifyCompat({ expo: "55.0.0", reactNative: "0.82.0" }).classification).toBe("unknown")
