...
const seedExpo = (fs: Fs["Type"]) =>
  fs
    .writeFile(`${ROOT}/package.json`, JSON.stringify({ dependencies: { expo: "54.0.0" } }))
    .pipe(Effect.orDie)
...
describe("AC-008 bridge install/remove require a confirmation token", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* bridgeInstall(ROOT, {}).pipe(
          Effect.provide(makeLayer(c, fs))
        )
  ...
  it.effect(
    "AC-008 install with allow but NO confirmation token still denies (0× writes)",
    () =>
      Effect.gen(function* () {
        const c = yield* makeCounters
        const fs = yield* makeMemoryFs()
  ...
  it.effect(
    "AC-008 install WITH allow + matching token writes bridge.json + the source file",
    () =>
      Effect.gen(function* () {
        const c = yield* makeCounters
        const fs = yield* makeMemoryFs()
  ...
  it.effect(
    "AC-008 install followed by a fresh read reports `present`",
    () =>
      Effect.gen(function* () {
        const c = yield* makeCounters
        const fs = yield* makeMemoryFs()
  ...
  it.effect("AC-008 remove with NO token deletes ZERO files (0× capability calls)", () =>
    Effect.gen(function* () {
      ...
      expect(payload.status.status).toBe("present")
      expect(payload.plan.deletes).toEqual([
        paths.metadata,
        paths.source,
  ...
  it.effect(
    "AC-008 remove WITH allow + token deletes both files plus the legacy .expo-ios fallback",
    () =>
      Effect.gen(function* () {
        const c = yield* makeCounters
        const fs = yield* makeMemoryFs()
