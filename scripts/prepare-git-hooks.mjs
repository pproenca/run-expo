...
const defaultPackageRoot = join(scriptDir, "..")
...
export function configurePrepareGitHooks(params = {}) {
  const cwd = params.cwd ?? defaultPackageRoot
  ...
  const worktree = runGit(spawn, gitBin, ["rev-parse", "--is-inside-work-tree"], cwd, ["ignore", "pipe", "ignore"])
  ...
  const configured = runGit(spawn, gitBin, ["config", "core.hooksPath", "git-hooks"], cwd, ["ignore", "ignore", "pipe"])
