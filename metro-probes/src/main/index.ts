...
class HermesCdpClient {
  ...
  private readonly pending = new Map<number, {
    resolve: (value: HermesEvaluationResult) => void;
    timeout: ReturnType<typeof setTimeout>;
  }>();

  constructor(private readonly webSocketDebuggerUrl: string) {}

  connect({ timeoutMs }: { timeoutMs: number }): Promise<void> {
  ...
  call(method: string, params: Record<string, unknown>, { timeoutMs }: { timeoutMs: number }): Promise<HermesEvaluationResult> {
    ...
    return new Promise((resolve) => {
      const timeout = setTimeout(() => {
        ...
        resolve({ error: `Timed out waiting for ${method}.` });
      }, timeoutMs);
  ...
  private handleMessage(event: MessageEvent): void {
    ...
    pending.resolve(record.error ? { error: formatError(record.error) } : record as HermesEvaluationResult);
  }
}
