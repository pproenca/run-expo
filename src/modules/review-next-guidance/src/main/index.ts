...
export function reviewCommandSuggestions(args: ReviewCommandSuggestionArgs): string[] {
  const base = [
    `expo-ios --json ux-context --cwd ${shellArg(args.cwd)} --metro-port ${args.metroPort}${args.componentFilter ? ` --component-filter ${shellArg(args.componentFilter)}` : ""}`,
  ];
  if (
    ...
    base.push(
      ...
      `expo-ios --json review-overlay scaffold --cwd ${shellArg(args.cwd)}`,
      `expo-ios --json review-overlay prepare --cwd ${shellArg(args.cwd)} --serve true`,
      `expo-ios --json review-overlay read --cwd ${shellArg(args.cwd)}`,
    );
  }
  if (args.flags.changedGesture || args.stage === "interaction") {
    base.push(
      `expo-ios --json trace --action start --metro-port ${args.metroPort}${args.componentFilter ? ` --component-filter ${shellArg(args.componentFilter)}` : ""}`,
...
export function reviewStopConditions(args: { flags: ReviewFlags; verifierRule?: string | null }): string[] {
  const stops: string[] = [];
...
export function verifierRuleMatchesChangedWorkflow(rule: string | null | undefined, flags: ReviewFlags): boolean {
