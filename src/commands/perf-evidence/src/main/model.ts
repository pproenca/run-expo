...
export function normalizePerfBridgePayload(value: any, action: string): Record<string, any> {
  ...
  const metrics = (value.metrics ?? []).map((metric: any) =>
    perfMetric({
      name: metric.name,
      value: metric.value,
      unit: metric.unit,
...
export function normalizePerfReport(
  runtimePayload: unknown,
  nativeSummary: PerfNativeSummary | null,
): PerfReport {
  ...
  const worstCommit = renders.reduce<PerfRenderCommit | null>(
    (worst, commit) =>
      Number(commit.durationMs ?? commit.actualDuration ?? 0) >
      Number(worst?.durationMs ?? worst?.actualDuration ?? 0)
        ? commit
        : worst,
    null,
  );
  if (worstCommit && Number(worstCommit.durationMs ?? worstCommit.actualDuration ?? 0) >= 16.7) {
    findings.push({
      type: "render-cost",
      severity:
        Number(worstCommit.durationMs ?? worstCommit.actualDuration ?? 0) >= 50 ? "high" : "medium",
      summary: "React render commit exceeded one frame budget.",
      evidence: worstCommit,
    });
  }
  const droppedFrames = Number(
    runtime?.frames?.droppedFrameCount ??
      frames.filter((frame) => Number(frame.deltaMs) > 33.4).length,
  );
  if (droppedFrames > 0) {
    findings.push({
      type: "frame-jank",
      ...
      evidence: {
        ...
        worstFrameMs: runtime?.frames?.worstFrameMs ?? null,
      },
    });
  }
  ...
  return {
    ...
    limitations: [
      ...(!renders.length
        ? ["Render cost is unavailable because no React Profiler commit records were returned."]
        : []),
      ...(!frames.length
        ? ["Frame jank is unavailable because no frame samples were returned."]
        : []),
      ...(!requests.length
        ? ["Network attribution is unavailable because no request rows were returned."]
        : []),
    ],
  };
}
...
export async function perfContext({
  args,
  projectRoot,
  metro,
  target = null,
...
export function perfOverallConfidence(
  metrics: Array<Pick<PerfMetric, "confidence">>,
): PerfConfidence {
