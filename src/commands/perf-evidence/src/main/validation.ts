...
export function perfValidation(payload: Record<string, any>, action: string): RealValidation {
  ...
  const hasNetwork =
    metrics.some((metric) => /network/i.test(String(metric.name)) && Number(metric.value) > 0) ||
    (Array.isArray(payload.requests) && payload.requests.length > 0) ||
    (Array.isArray(payload.runtime?.network?.requests) &&
      payload.runtime.network.requests.length > 0);
  const hasRender =
    metrics.some(
    ...
    (Array.isArray(payload.runtime?.renders?.commits) &&
      payload.runtime.renders.commits.length > 0);
  const hasFrames =
    metrics.some(
      (metric) =>
        /frame/i.test(String(metric.name)) &&
        Number(metric.value) > 0 &&
        !/available/.test(String(metric.name)),
    ) ||
  ...
  const placeholderMetric = metrics.some(
    (metric) =>
      /available$|bridge\.available|interaction\.duration/.test(String(metric.name)) &&
      ...
      ? [
          {
            ...
            recommendedFix:
              "Run network requests after a real interaction or mount the metadata network bridge.",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            recommendedFix:
              "Mount the dev-only Profiler wrapper or run rn renders start/read/stop with bridge commit records.",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            recommendedFix:
              "Start frame sampling before exercising the interaction and rerun perf frames/report.",
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            reason: hasNativeArtifact
              ? "Native sample artifact was parsed, but PID and sample duration were not attached to this evidence."
              : "No parseable native sample artifact was available.",
            recommendedFix:
              "Run profiler start with --pid, --seconds, and --native-artifact, then pass that artifact to perf report.",
          },
        ]
      : []),
    ...(!releaseLike
      ? [
          {
            ...
            recommendedFix:
              "Repeat the profile against a preview or production build before making release performance claims.",
          },
        ]
      : []),
  ];
