...
export const TOOL_HANDLER_BINDINGS = COMMAND_SURFACE.filter(
  (entry, index, entries) =>
    entries.findIndex((candidate) => candidate.toolName === entry.toolName) === index,
).map(
  (entry) => [entry.toolName, entry.handlerSymbol] as const,
) satisfies readonly ToolHandlerBinding[];
