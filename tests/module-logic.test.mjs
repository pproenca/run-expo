...
describe("network evidence pure logic", () => {
  it("redacts secret-bearing request data and annotates HAR metadata", async () => {
    ...
    const normalized = normalizeNetworkEvidence(
      { available: true, source: "plugin-bridge", requests: [] },
      "requests",
    );
    ...
    const har = harFromNetworkRequests(redacted.requests, {
    ...
    assert.deepEqual(annotated.log._expoIos.redaction.headers.slice(0, 2), [
      "authorization",
      "cookie",
    ]);
  });
...
describe("perf validation pure logic", () => {
  it("distinguishes validated interaction evidence from placeholder-only evidence", async () => {
    const { perfValidation } = await importSourceModule(
      "perf-validation",
...
describe("Hermes CDP URL helpers", () => {
  it("normalizes loopback WebSocket candidates and Metro Origin headers", async () => {
    const { loopbackWebSocketCandidates, metroOriginForWebSocket } = await importSourceModule(
      "hermes",
      "src/platform/hermes-cdp-client/src/main/index.ts",
      ["loopbackWebSocketCandidates", "metroOriginForWebSocket"],
    );

    const candidates = loopbackWebSocketCandidates(
    ...
    assert.equal(
      metroOriginForWebSocket("ws://localhost:19000/inspector"),
...
describe("interaction command planning", () => {
  it("plans platform-specific gestures and simulator environment mutations", async () => {
    const { gestureCommandPlan, setEnvironmentPlan } = await importSourceModule(
      "interaction",
      "src/commands/interaction-actions/src/main/index.ts",
      ["gestureCommandPlan", "setEnvironmentPlan"],
    );
    ...
    const appearance = setEnvironmentPlan(
      "appearance",
      { value: "dark" },
      { udid: "sim-1", name: "iPhone" },
    );
    assert.deepEqual(appearance.command, ["xcrun", "simctl", "ui", "sim-1", "appearance", "dark"]);
    assert.throws(
      () => setEnvironmentPlan("appearance", { value: "sepia" }, { udid: "sim-1" }),
...
describe("policy gate negative paths", () => {
  it("keeps policy decisions fail-closed for device, write, and runtime-eval actions", async () => {
    const { decideActionPolicy } = await importSourceModule(
      "policy",
    ...
    assert.equal(
      decideActionPolicy({ action: "wait.fn", sideEffect: "runtime-eval" }).allowed,
      false,
    );
    assert.equal(
      decideActionPolicy({ action: "wait.fn", sideEffect: "runtime-eval", allowRuntimeEval: true })
        .allowed,
      true,
    );
  });
  ...
  it("denies route mutations before touching fake executors and allows explicit policy", async () => {
    const { openUrl } = await importSourceModule(
      "route-policy",
    ...
    const denied = parseToolJson(
      await openUrl({ platform: "android", url: "exp://example.test" }, { execFile }),
    );
  ...
  it("denies app lifecycle mutations before device resolution or subprocess execution", async () => {
    const { bootSimulator } = await importSourceModule(
      "app-policy",
  ...
  it("denies bridge-backed writes before Metro/Hermes and executes allowed state writes", async () => {
    const { stateCommand, storageCommand } = await importSourceModule(
      "bridge-policy",
      "src/commands/bridge-domain-actions/src/main/index.ts",
      ["stateCommand", "storageCommand"],
    );
    let metroCalls = 0;
    let evalCalls = 0;
    const deps = {
    ...
    const denied = parseToolJson(
      await storageCommand(
        { store: "async-storage", action: "set", key: "theme", value: "{}" },
        deps,
      ),
    );
    assert.equal(denied.code, "policy-denied");
    ...
    const allowed = parseToolJson(
      await stateCommand({ action: "save", name: "checkpoint", actionPolicy: "policy.json" }, deps),
    );
  ...
  it("denies modal mutations before Metro/Hermes and allows explicit dialog policy", async () => {
    const { dialogCommand, sheetCommand } = await importSourceModule(
      "modal-policy",
      "src/commands/modal-blocker-actions/src/main/index.ts",
      ["dialogCommand", "sheetCommand"],
    );
    let metroCalls = 0;
    let evalCalls = 0;
    const deps = {
    ...
    const allowed = parseToolJson(
      await dialogCommand({ action: "dismiss", actionPolicy: "policy.json" }, deps),
    );
    assert.equal(allowed.available, true);
    assert.equal(allowed.policy.allowed, true);
    assert.equal(metroCalls, 1);
  ...
  it("denies open-dev-menu before Metro broadcast, device resolution, or xcrun", async () => {
    const { openIosDevMenu } = await importSourceModule(
      "dev-menu-policy",
